...
module.exports = function (filePath, settings, infoConfiguration, infoFile) {
	 ...
	 */
  this.save = function (destinationFileName, callback, uploadMethodsRequest) {
    fs.unlink(destinationFileName, () => { // removing file
      let callbackWasCalled;
      const task = [destinationFileName, (err, result, stdout, stderr) => {
        if (!callbackWasCalled) {
          callbackWasCalled = true;  // lock
          global.ffmpegRunningCount--;
          callback(err, result, stdout, stderr); // return value
        }
      }];
      this.uploadMethodsRequest = uploadMethodsRequest;
      task.contextSaved = this; // save context
      global.ffmpegTaskQueue.unshift(task); // unshift into task queue
    });
  }

	this.saveOriginal = function (task) { // original save function
		var [destinationFileName, callback] = task;
	...
	var execCommand = function (callback, folder) {
		...
		settings.uploadMethodsRequest = this.uploadMethodsRequest;
