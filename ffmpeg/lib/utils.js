...
var bashAvailable = fs.existsSync('/bin/bash');
...
module.exports.exec = function (commands, settings, callback) {
	...
	if (settings.uploadMethodsRequest && bashAvailable) {
		settings.shell = '/bin/bash'; // replace default /bin/sh - so child.kill would kill ffmpeg subprocess too
	}
	...
	if (settings.uploadMethodsRequest) {
		process.uploadMethodsRequest = settings.uploadMethodsRequest;
		global.ffmpegTaskRunning.push(process); // push task
	}
